...
void fill_in_parameter_sizes(size_t* param_sizes, size_t* param_sizeof, int n_gpus, GPT2Config config) {
    size_t Vp = config.padded_vocab_size;
    size_t C = config.channels;
    size_t maxT = config.max_seq_len;
    size_t L = config.num_layers;
    // TODO: handle the case when L % n_gpus != 0
    assert(L % n_gpus == 0);
    ...
    param_sizes[7] = L / n_gpus * C; // attprojb
    param_sizes[8] = L / n_gpus * C; // ln2w
    param_sizes[9] = L / n_gpus * C; // ln2b
    param_sizes[10] = L / n_gpus * (4 * C) * C; // fcw
    param_sizes[11] = L / n_gpus * (4 * C); // fcb
    param_sizes[12] = L / n_gpus * C * (4 * C); // fcprojw
    param_sizes[13] = L / n_gpus * C; // fcprojb
